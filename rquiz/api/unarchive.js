...
export default async function handler(request, response) {
    ...
    try {
        ...
        delete results[firstName]._archivedAt;
        ...
        return response.status(200).json({ message: 'Student unarchived successfully' });
    } catch (error) {
        return response.status(500).json({ message: 'Error unarchiving student' });
    }
}
